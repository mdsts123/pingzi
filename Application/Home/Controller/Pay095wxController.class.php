...
class Pay095wxController extends HomeController {
  /**
   * 金阳支付入款
   ...
   */
	protected function _initialize(){
        $this->conf = M('Payapi')->cache('Pay095wx_conf')->where("payclass='Pay095wx'")->find();
    }
    public function pay() {
       ...
       $data["p2_paytype"] = isMobile()?"WEIXINWAP":'WEIXIN';
       $data["p3_paymoney"] = sprintf("%0.02f", $order['amount']);
       $data["p4_orderno"] = $order['orderno'];
       $data["p5_callbackurl"] = U('Pay095wx/notify','',true,true);//异步通知
       $data["p6_notifyurl"] = U('Pay095wx/hrefback','',true,true);//同步通知
