...
class YmfzfbController extends HomeController {
    protected function _initialize(){
        $this->conf = M('Payapi')->where("payclass='Ymfzfb'")->find();//->cache('Ymfzfb_conf')
    }
    ...
    public function pay()
	{
		...
		if (isMobile()) {
			$data = array(
					...
					'tradeDate' => date("Ymd"),
					'amt' => $order['amount'],
					'merchUrl' => U('Ymfzfb/notify', '', false, true),
					'merchParam' => 'abcd',
		...
		$data['signMsg'] = $sign;
		if (isMobile()) {
			$data['choosePayType'] = '9';//支付类型
		}else
			$data['choosePayType'] = '10';//支付类型
