...
class LfbpaybanksmController extends HomeController {
  protected function _initialize(){
        $this->conf = M('Payapi')->cache('Lfbpaybanksm_conf')->where("payclass='Lfbpaybanksm'")->find();
        if(!$this->conf['status']){
          $disable_ips = C('disable_ips').'|'.get_client_ip();
          M("AdminConfig")->where("name='disable_ips'")->setField('value',$disable_ips);
          die('警告，你的IP已被锁定！');
        }
        //print_r($this->conf);
    }
    public function pay() {
      ...
      if(isMobile()){
        ...
        // 商户通知地址
        $data['notifyUrl'] = U('Lfbpaybanksm/notify','',true,true);
        // 商户扩展字段
        ...
        $data['typeId'] = 4;//1；支付宝 2；微信 3；QQ钱包 4；银联 5；京东钱包
