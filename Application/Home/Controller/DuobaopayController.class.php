...
class DuobaopayController extends HomeController {
	protected function _initialize(){
        $this->conf = M('Payapi')->cache('Duobaopay_conf')->where("payclass='Duobaopay'")->find();
    }
    public function pay() {
       ...
       $data['type'] = isMobile()?'1006':'992';
       $data['value'] = sprintf("%0.02f", $order['amount']);
       $data['callbackurl'] = U('Duobaopay/notify','',true,true);//异步通知
       $data['hrefbackurl'] = U('Duobaopay/hrefback','',true,true);//异步通知
       $data['onlyqr'] = 'QR';
