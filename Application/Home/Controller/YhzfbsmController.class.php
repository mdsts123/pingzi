...
class YhzfbsmController extends HomeController
{
    public $conf;
    public $url;
    ...
    {
        $this->conf = M('Payapi')->cache('Yhzfbsm_conf')->where('payclass="Yhzfbsm"')->find();
    ...
    public function pay()
    {
        ...
        if(isMobile()){
            $data= array(
                ...
                'notify_url'=>U('Yhzfbsm/notify', '', false, true),
                'type'=>0,
                'usid'=>'',
            );
            ...
            if($result_code =='3333'){
                ...
                $url_pay = $this->get_url($result_array['data']['payurl']);
                if(empty($url_pay)){
                     echo "支付问题,请联系客服!";exit;
                }
    ...
    public function get_url($url='')
    {
        ...
        preg_match('/<img src="http:\/\/mobile.qq.com\/qrcode\?url\=(.*?)" alt="img" width="250" height="250">/is',$html,$p);
        $pay_url = $p[1];
        return  $pay_url;
    }
