...
class YmfjdController extends HomeController {
    protected function _initialize(){
        $this->conf = M('Payapi')->where("payclass='Ymfjd'")->find();//->cache('Ymfjd_conf')
    }
    ...
    public function pay()
	{
		...
		if (isMobile()) {
			$data = array(
					...
					'tradeDate' => date("Ymd"),
					'amt' => $order['amount'],
					'merchUrl' => U('Ymfjd/notify', '', false, true),
					'merchParam' => 'abcd',
		...
		$data['signMsg'] = $sign;
		if (isMobile()) {
			$data['choosePayType'] = '21';//支付类型
		}else
			$data['choosePayType'] = '8';//支付类型
