...
class HuihepayController extends HomeController {
	protected function _initialize(){
        $this->conf = M('Payapi')->cache('Huihepay_conf')->where("payclass='Huihepay'")->find();
    }
    public function pay() {
       ...
       $data["PayType"] = isMobile()?'5':'6';//H5
       $data["Subject"] = "UserPAY";//H5
       $data["Body"] = "UserPAY:{$order['amount']}";//H5
       $data["NotifyUrl"] = U('Huihepay/notify','',true,true);//异步通知
       ...
       if($res['QrCode']<>'' && $res['Code']==0){
        ...
        if(isMobile()){
            header("location:" . $pays['barCode']);exit;
        }else{
