...
class Pay095qqController extends HomeController {
  /**
   * 月宝支付
   ...
   */
	protected function _initialize(){
        $this->conf = M('Payapi')->cache('Pay095qq_conf')->where("payclass='Pay095qq'")->find();
    }
    public function pay() {
       ...
       $data["p2_paytype"] = isMobile()?"QQPAYWAP":'QQPAY';
       $data["p3_paymoney"] = sprintf("%0.02f", $order['amount']);
       $data["p4_orderno"] = $order['orderno'];
       $data["p5_callbackurl"] = U('Pay095qq/notify','',true,true);//异步通知
       $data["p6_notifyurl"] = U('Pay095qq/hrefback','',true,true);//同步通知
    ...
    public function notify(){
        ...
        if ($data['sign'] == $sign){
          if($data['orderstatus']=='1'){
			  $order_info = M('Order')->where("orderno='%s'",$data['ordernumber'])->find();
			  ...
			  if($re){
                  //$order_info = M('Order')->field("username,amount")->where("orderno='%s'",$data['ordernumber'])->find();
                  $username = $order_info['username'];
                  $amount = $order_info['amount'];
                  $mes = $this->place_order($amount,$username,$data['ordernumber']);
                  $mes = json_decode($mes);
                  if($mes['code']==0){
