...
class ListBuilder extends ControllerController
{
   ...
   * @author jry <93058680@qq.com>
   */
  public function addTableColumn($name, $title, $type = null, $param = null, $wm = false)
  {
    $column = array(
      'name'  => $name,
      'title' => $title,
  ...
  public function addRightButton($type, $attribute = null)
  {
    switch ($type) {
      ...
      case 'recycle':
        ...
        $my_attribute['recycle1']['title'] = '已付';
        ...
        $my_attribute['recycle-1']['title'] = '未付';
  ...
  public function display()
  {
    ...
    foreach ($this->_table_data_list as &$data) {
        ...
        foreach ($this->_right_button_list as $right_button) {
          ...
          if ($right_button['type'] === 'recycle') {
            if ($data['payzt'] === '0') {
              ...
              $right_button = $right_button['recycle-1'];
            }
          }
      ...
      if (!empty($data['cmit_time'])) {
        $data['cmit_time'] =  time_format($data['cmit_time'], 'Y-m-d H:i:s');
      }
      // 推广订单 - 操作时间
      if (!empty($data['operatime'])) {
        $data['operatime'] =  time_format($data['operatime'], 'Y-m-d H:i:s');
      }
      // 用户注册时间转换
      if (!empty($data['create_time'])) {
        $data['create_time'] =  time_format($data['create_time'], 'Y-m-d H:i:s');
      }
...
//        }
      ...
      // 状态
      if (0 == $data['pay_status']) {
        $data['pay_status_name'] = "未处理";
      } elseif (1 == $data['pay_status']) {
        $data['pay_status_name'] = "充值成功";
      } elseif (2 == $data['pay_status']) {
        $data['pay_status_name'] = "充值失败";
      }
      ...
      foreach ($this->_table_column_list as &$column) {
        switch ($column['type']) {
          ...
          case 'avatar':
            ...
            break;
          case 'picture':
            $data[$column['name']] = '<img class="picture ad-order-img" src="' . ($wmdata[2]["wu"] == false ? get_cover($data[$column['name']]) : $data[$column['name']]) . '">';
            break;
          case 'pictures':
